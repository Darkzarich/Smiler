...
test.describe('Auth state', () => {
  ...
  test('Shows auth-ed user menu', async ({ page, isMobile, context }) => {
    ...
    await expect(page.getByTestId('user-rating')).toContainText(auth.rating.toString());
    await expect(page.getByTestId('user-followers-amount')).toContainText(auth.followersAmount.toString());
  ...
  test('Shows not auth-ed state in the user menu on logout', async ({ page, isMobile, context }) => {
    ...
    const logoutRequest = page.waitForRequest((res) => res.url().includes('/users/logout') && res.method() === 'POST');
...
test.describe('Sign In validation', () => {
  ...
  test('Shows no validation errors by default', async ({ page }) => {
    ...
    await expect(page.getByTestId('user-signin-password-error')).not.toBeVisible();
  });

  test('Errors on not a valid email', async ({ page }) => {
    await page.getByTestId('user-signin-email').fill('1234');
    await expect(page.getByTestId('user-signin-email-error')).toContainText('Email is not valid');
  });

  test('Errors on emptied after entering email', async ({ page }) => {
    await page.getByTestId('user-signin-email').fill('1234');
    await page.getByTestId('user-signin-email').clear();
    await expect(page.getByTestId('user-signin-email-error')).toContainText('Email can\'t be empty');
  });

  test('Errors if password length is not minimum 6', async ({ page }) => {
    await page.getByTestId('user-signin-password').fill('1234');
    await expect(page.getByTestId('user-signin-password-error')).toContainText('Password length must be minimum 6');
  });

  test('Errors on emptied after entering password', async ({ page }) => {
    await page.getByTestId('user-signin-password').fill('1234');
    await page.getByTestId('user-signin-password').clear();
    await expect(page.getByTestId('user-signin-password-error')).toContainText('Password can\'t be empty');
  });
});
...
test.describe('Sign Up validation', () => {
  ...
  test('Shows no validation errors by default', async ({ page }) => {
    ...
    await expect(page.getByTestId('user-signup-password-error')).not.toBeVisible();
    await expect(page.getByTestId('user-signup-confirm-error')).not.toBeVisible();
  });

  test('Errors on not a valid email', async ({ page }) => {
    await page.getByTestId('user-signup-email').fill('1234');
    await expect(page.getByTestId('user-signup-email-error')).toContainText('Email is not valid');
  });

  test('Errors on emptied after entering email', async ({ page }) => {
    await page.getByTestId('user-signup-email').fill('1234');
    await page.getByTestId('user-signup-email').clear();
    await expect(page.getByTestId('user-signup-email-error')).toContainText('Email can\'t be empty');
  });

  test('Errors if login length is not minimum 3', async ({ page }) => {
    await page.getByTestId('user-signup-login').fill('te');
    await expect(page.getByTestId('user-signup-login-error')).toContainText('Login length must be 3-10');
  });
  ...
  test('Errors if password length is not minimum 6', async ({ page }) => {
    ...
    await expect(page.getByTestId('user-signup-password-error')).toContainText('Password length must be minimum 6');
  });

  test('Errors on emptied after entering password', async ({ page }) => {
    await page.getByTestId('user-signup-password').fill('1234');
    await page.getByTestId('user-signup-password').clear();
    await expect(page.getByTestId('user-signup-password-error')).toContainText('Password can\'t be empty');
  });

  test('Errors if confirm length is not minimum 6', async ({ page }) => {
    await page.getByTestId('user-signup-confirm').fill('1234');
    await expect(page.getByTestId('user-signup-confirm-error')).toContainText('Password confirm length must be minimum 6');
  });

  test('Errors on emptied after entering confirm', async ({ page }) => {
    await page.getByTestId('user-signup-confirm').fill('1234');
    await page.getByTestId('user-signup-confirm').clear();
    await expect(page.getByTestId('user-signup-confirm-error')).toContainText('Password confirm can\'t be empty');
  });
  ...
  test('Errors if password and confirm are not equal', async ({ page }) => {
    ...
    await expect(page.getByTestId('user-signup-confirm-error')).toContainText('Password and password confirm must be equal');
  });
});
