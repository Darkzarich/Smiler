...
- **Aug 2026 — commit conventions** — The repo adopts **Conventional Commits** (the Angular flavour), validated on every commit by **[commitlint](https://github.com/conventional-changelog/commitlint)** in a new `commit-msg` hook that also spellchecks the message, with commit scopes restricted to the workspace packages. The git hooks are rebalanced at the same time: linting stays at commit time through lint-staged, while pre-push runs only the test suites instead of repeating the repo-wide lint passes.
- **Sep 2026 — styling** — The frontend drops **Sass for plain CSS on [PostCSS](https://github.com/postcss/postcss)**. The March color-variable refactor had already moved every color to CSS custom properties, which left the preprocessor supplying only nesting and one breakpoint mixin: no Sass variables, color functions, loops or maps remained anywhere in the app. **[postcss-nested](https://github.com/postcss/postcss-nested)** takes over the nesting — the Sass flavour rather than the CSS spec's, because BEM `&__element` concatenation is a Sass invention the standard cannot express — and the breakpoints become `@custom-media` definitions that **[@csstools/postcss-global-data](https://github.com/csstools/postcss-plugins)** injects into every style block, so components no longer import anything to be responsive. The compiled CSS was diffed bundle-by-bundle against the Sass output to keep the change invisible.
- **Sep 2026 — pictures** — Pictures added by url stop being hotlinked. A pasted link used to be stored as-is and served to every reader from the host it pointed at, which handed that host the IP address of everyone who opened the post, left the bytes under its control indefinitely, and escaped the size limit an uploaded file has — the url was validated, but validating a string says nothing about what the response contains. The backend now **downloads the picture, re-encodes it with [sharp](https://github.com/lovell/sharp) and stores it** alongside file uploads, for post sections and for avatars alike, so the encoded size, the pixel count and the stored dimensions are all bounded. Fetching a url the user chose is an SSRF sink, so it is guarded on both sides: address literals and names reserved for the local network are refused before a socket opens, and every name is checked again at connect time against the address it actually resolved to, which is what closes DNS rebinding. Redirects are followed by hand and revalidated hop by hop.
- **Sep 2026 — spoilers** — Any post section — text, picture or video — can be published behind a **spoiler**: one toggle in the editor, which previews the section the way readers will see it, and a blurred veil on the rendered post that lifts on click, with the covered content kept out of reach of the keyboard and screen readers until it does.
- **Sep 2026 — observability** — Backend logging is reworked around **structured stdout**. **[Winston](https://github.com/winstonjs/winston)** had been writing `logs/*.log` from every cluster worker at once, and its rotation counts bytes per process, so the workers renamed the file out from under each other and lost lines; stdout is now the only transport and Docker's `json-file` driver owns rotation. Every line is one JSON object carrying a request id, the matched route template and the acting user.
